...
namespace VectorVertex {
    Scope<FrameBuffer> FrameBuffer::Create(int width, int height) {
        switch (RenderAPI::GetAPI()) {
            case RenderAPI::API::None: VV_CORE_WARN("VectorVertex is currently not support for nonGraphicsAPI!"); return nullptr;
            case RenderAPI::API::OpenGL: return CreateScope<OpenGLFBO>(static_cast<int>(width), static_cast<int>(height));
        }
