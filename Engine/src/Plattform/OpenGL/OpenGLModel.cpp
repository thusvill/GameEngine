...
OpenGLModel::OpenGLModel(const char *file): file(file) {
...
std::vector<unsigned char> OpenGLModel::getData() {
    ...
    std::vector<unsigned char> out_data(bytesText.begin(), bytesText.end());
    return out_data;
}
std::vector<float> OpenGLModel::getFloats(VectorVertex::json accessor) {
...
std::vector<GLuint> OpenGLModel::getIndices(VectorVertex::json accessor) {
...
std::vector<Vertex> OpenGLModel::assembleVertices(std::vector<glm::vec3> positions, std::vector<glm::vec3> normals, std::vector<glm::vec2> texUVs) {
